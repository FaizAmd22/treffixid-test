...
const BehaviorCard: React.FC<BehaviorCardProps> = ({ data, previousPercentage }) => {
  const [percentageChange, setPercentageChange] = useState<number>(0);
  const [changeDirection, setChangeDirection] = useState<"increase" | "decrease" | "no-change">(
    "no-change"
  );

  useEffect(() => {
    if (previousPercentage !== undefined) {
      const change = ((data.percentage - previousPercentage) / previousPercentage) * 100;
  ...
  return (
    <>
        ...
        <div>
          {changeDirection === "increase" ? (
            <div className="w-fit px-2 flex justify-center items-center text-green-500 bg-green-50 border-2 border-green-500 rounded-full">
              ...
              <span className="ml-1 bg-transparent">{percentageChange.toFixed(1)}%</span>
            </div>
            ...
            <div className="w-fit px-2 flex justify-center items-center text-red-500 bg-red-50 border-2 border-red-500 rounded-full">
              ...
              <span className="ml-1 bg-transparent">{Math.abs(percentageChange).toFixed(1)}%</span>
            </div>
          ) : (
      ...
      <div className="mt-4">
        <Line percent={data.percentage} strokeWidth={8} strokeColor="#1DBF73" trailColor="#D9D9D9" trailWidth={8} strokeLinecap="round" />
      </div>

      <div className="grid grid-cols-2 gap-4 mt-6 text-center">
        {data.dataMetric.map((metric, index) => (
          <div key={index} className="flex gap-4">
            <p className="font-semibold text-gray-800 w-fit bg-slate-100 p-2 px-3 rounded-lg">{metric.point}</p>
